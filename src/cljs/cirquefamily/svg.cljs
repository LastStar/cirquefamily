  ...
  (fn []
    (let [logo-clicked (frame/subscribe [:logo-clicked])
      ...
      [:svg.eight.wide.column
       ...
       (print @logo-clicked)
       (if (and @logo-clicked (not @show-subscribe))
         [:g
          {:on-click #(frame/dispatch [:show-subscribe])
           :id "text"
           :stroke "none"
           :stroke-width "1"
           :fill "none"
...
(defn home [part background]
  (let [dispatcher (fn [p] (frame/dispatch [:home-part-changed-to p]))
        active-on (fn [p] {:class (when (= part p) "active")})]
    [:svg
     {:id "home"
      :xmlns "http://www.w3.org/2000/svg"
     ...
     [:g#texts
      ...
      [:g.inner
       (active-on :inner)
       [:text {:x 100 :y 20} "family"]]
      [:g.center
       (active-on :center)
       [:text {:x 100 :y 20} "friends"]]
      [:g.outer
       (active-on :outer)
       [:text {:x 100 :y 20} "world"]]]]))
